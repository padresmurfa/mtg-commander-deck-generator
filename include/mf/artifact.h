 ...
 *
 * Buffered with a fixed buffer allocated at open, so writing a record never
 * allocates — see the no_io_in_loop invariant (design §10). Flush points are
 * explicit; nothing here flushes on its own except when the buffer fills or a
 * record is too large to buffer at all. */
...
/* Flushes, closes, frees. Safe on NULL. */
