...
/* One value at a time out of a JSON array that does not fit in memory.
 *
 * Scryfall's bulk export is a single array of roughly 110,000 printings and
 * several hundred megabytes. `mf_json_parse` reads a whole document into an
 * arena, which for that input would cost gigabytes to hold a result that is
 * read once, field by field, and thrown away.
 *
...
/* `open` owns the file and closes it; `open_stream` borrows one and does not.
   Neither reads past the opening bracket. */
